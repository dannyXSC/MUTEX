...
class MLMTaskDataset(SequenceVLDataset):
    def __init__(
        ...
        if self.mask_inst and self.inst_tokens is not None:
            ...
            for task_spec_id, inst_token in enumerate(
                self.inst_tokens["input_ids"]
            ):  # number of possible instructions
                inst_list = []
                for inst_ind, sent_tokens in enumerate(
                    inst_token
                ):  # number of sentences per instruction
                    if torch.sum(
                        self.inst_tokens["attention_mask"][task_spec_id][
                            inst_ind
                        ].long()
                    ):
                        desc_list = []
                        for desc_ind, token in enumerate(
                            sent_tokens
                        ...
                        assert (
                            len(desc_list) > 0
                        ), "No tokens found in sentence {}, {}, {}".format(
                            task_spec_id, inst_ind, sent_tokens
                        )
        ...
        if self.mask_gl and self.gl_tokens is not None:
            ...
            for task_spec_id, gl_token in enumerate(
                self.gl_tokens["input_ids"]
            ):  # number of possible goal language
                gl_list = []
                for mgm_index, token in enumerate(gl_token):
                    if self.gl_tokens["attention_mask"][task_spec_id][
                        mgm_index
                    ] and not (token in self.gl_tokens["stopword_tokens"]):
    ...
    def generate_visual_specifications(self):
        ...
        if "vid" in self.task_spec_modalities:
            task_spec_id = random.randint(
                0, len(self.visual_spec["vid_task_spec"]) - 1
            )  # sample inclusive
            vid_task_spec = self.visual_spec["vid_task_spec"][task_spec_id].clone()
            vid_task_spec_mask = self.visual_spec["vid_task_spec_mask"][
                task_spec_id
            ].clone()
        ...
        if "img" in self.task_spec_modalities:
            task_spec_id = random.randint(
                0, len(self.visual_spec["img_task_spec"]) - 1
            )  # sample inclusive
            # don't clone here, since it is a very big tensor
            img_spec = self.visual_spec["img_task_spec"][
                task_spec_id
            ]  # .clone() ## [1, 50, 768]: Specifies the final goal
            ...
            img_spec_mask = torch.ones(
                self.visual_spec["img_task_spec_mask"][task_spec_id].shape
            )  ## [1,50]
            img_spec_mask = img_spec_mask.unsqueeze(dim=2).repeat(
                1, 1, img_spec_mask.shape[-1]
            )  ## [1,50,50], all for clip
            # don't clone here, since it is a very big tensor
            gt_img_spec = self.visual_spec["img_task_spec"][task_spec_id]  # .clone()

            if self.mask_img:
                mrm_indices = range(
                    1, img_spec_mask.shape[-1], 10
                )  ## we cannot mask 0th feature because clip uses that as pooled feature, mask every 10th feature
                mrm_indices = sorted(random.sample(mrm_indices, 1))
                img_spec_mask[:, :, mrm_indices] = float(
                    "-inf"
                )  ## the way attention is defined in CLIP; attn_weights + attention_mask
            else:
                mrm_indices = []
    ...
    def generate_language_specifications(self):
        ...
        if "inst" in self.task_spec_modalities:
            task_spec_id = random.randint(
                0, len(self.inst_tokens["input_ids"]) - 1
            )  # sample inclusive
            input_ids = self.inst_tokens["input_ids"][task_spec_id].clone()
            gt_ids = self.inst_tokens["input_ids"][task_spec_id].clone()
            attention_mask = self.inst_tokens["attention_mask"][
                task_spec_id
            ].clone()  ## don't make changes in the original task tokens
            ...
            if self.mask_inst:
                ...
                mim_indices = sorted(
                    random.sample(range(total_valid_inst), 2)
                )  # pick any two instructions
                desc_indices = []
                for instruct_index in mim_indices:
                    ind = sorted(
                        random.sample(
                            self.mim_sample_indices[task_spec_id][instruct_index], 1
                        )
                    )[0]
            ...
            attention_mask[mim_indices, desc_indices] = 0.0
            ...
            return_dict["inst_emb_mask"] = torch.ones(
                return_dict["inst_tokens"]["attention_mask"].shape[:-1]
            )
        ...
        if "gl" in self.task_spec_modalities:
            ...
            if self.gl_tokens is None:
                return_dict["gl_emb"] = self.gl_emb[
                    task_spec_id
            ...
            attention_mask = self.gl_tokens["attention_mask"][
                task_spec_id
            ].clone()  ## don't make changes in the original task tokens

            if self.mask_gl:
                mgm_indices = sorted(
                    random.sample(self.mgm_sample_indices[task_spec_id], 1)
                )  ## TODO: Samples only one index at a time
            else:
                return_dict["gl_emb"] = self.gl_emb[task_spec_id].unsqueeze(
                    dim=0
                )  ## adding time dimension
                mgm_indices = []

            attention_mask[mgm_indices] = (
                0  ## change attention mask for tokens that are masked
            )
            ...
            return_dict["gt_gl_ids"] = torch.Tensor(
                [gt_ids[pred_val] for pred_val in mgm_indices]
            ).long()  ## for the mgm loss
        return return_dict
    ...
    def generate_audio_specification(self):
        ...
        if "ai" in self.task_spec_modalities:
            task_spec_id = random.randint(
                0, len(self.ai_task_spec["ai_task_spec"]) - 1
            )  # sample inclusive
        ...
        if "ag" in self.task_spec_modalities:
            task_spec_id = random.randint(
                0, len(self.ag_task_spec["ag_task_spec"]) - 1
            )  # sample inclusive
    ...
    def __getitem__(self, idx):
        ...
        if "lang" in self.task_spec_modalities:
            ...
            attention_mask = self.task_tokens[
                "attention_mask"
            ].clone()  ## don't make changes in the original task tokens
            ...
            if self.mask_lang:
                ...
                mlm_indices = random.sample(
                    mlm_indices, 1
                )  ## TODO: Samples only one index at a time
                del return_dict[
                    "task_emb"
                ]  ## Removing task embeddings so that it does not use it in the algo.
            else:
                mlm_indices = []
            ...
            task_tokens["gt_ids"] = torch.Tensor(
                [gt_ids[pred_val] for pred_val in mlm_indices]
            ).long()
