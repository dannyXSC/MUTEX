...
class Multitask(Sequential):
    ...
    def __init__(self, n_tasks, cfg, logger, **policy_kwargs):
        super().__init__(n_tasks=n_tasks, cfg=cfg, logger=logger, **policy_kwargs)
        print(
            "# Actor Learnable Params: %d"
            % sum(p.numel() for p in self.policy.parameters() if p.requires_grad)
        )
    ...
    def train_loop(self, dataloader, epoch):
        ...
        for idx, data in tqdm(
            enumerate(dataloader), total=len(dataloader), disable=self.cfg.train.debug
        ):
            loss, log_info = self.observe(data)
            training_loss += loss
            train_log_dict = update_log_dict(
                train_log_dict, log_info, prefix="train", mode="run_avg"
            )
    ...
    def val_loop(self, dataloader, epoch):
        ...
        for idx, data in tqdm(
            ...
            val_loss += loss
            val_log_dict = update_log_dict(
                val_log_dict, log_info, prefix="val", mode="run_avg"
            )
            if self.cfg.train.debug:
    ...
    def learn_all_tasks(self, datasets, start_epoch=0, model_prefix=""):
        ...
        train_dataset, val_dataset = torch.utils.data.random_split(
            concat_dataset,
            [
                int(self.cfg.data.train_dataset_ratio * len(concat_dataset)),
                len(concat_dataset)
                - int(self.cfg.data.train_dataset_ratio * len(concat_dataset)),
            ],
        )
        ...
        model_checkpoint_name = os.path.join(
            self.model_dir, f"{model_prefix}multitask_model.pth"
        )
        ...
        for epoch in range(start_epoch, self.cfg.train.n_epochs + 1, 1):
            ...
            if (not self.cfg.train.debug) and (
                (epoch % self.cfg.train.val_every == 0)
                or (epoch == self.cfg.train.n_epochs)
            ):  ## Validation loop
                t0 = time.time()
                validation_loss, val_log_dict = self.val_loop(
                    val_dataloader, epoch=epoch
                )
                ...
                print(
                    f"[info] Epoch: {epoch:3d} | val loss: {validation_loss:5.2f} | best val loss: {best_val_loss:5.2f} | time: {(t1-t0)/60:4.2f}"
                )

                model_checkpoint_name_ep = os.path.join(
                    self.model_dir, f"{model_prefix}multitask_model_ep{epoch:03d}.pth"
                )
                ...
                torch.save(
                    np.array(losses),
                    os.path.join(self.experiment_dir, f"multitask_auc.log"),
                )
                torch.save(
                    np.array(val_losses),
                    os.path.join(self.experiment_dir, f"multitask_val_auc.log"),
                )
                if epoch == self.cfg.train.n_epochs:
                    break  # stop training after last validation run

            t0 = time.time()
            training_loss, train_log_dict = self.train_loop(
