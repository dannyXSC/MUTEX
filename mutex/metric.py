...
def raw_obs_to_tensor_obs(obs, task_emb, cfg):
    ...
    for k in range(env_num):
        data["obs"]["agentview_rgb"].append(
            ObsUtils.process_obs(
                torch.from_numpy(obs[k]["agentview_image"]), obs_key="agentview_rgb"
            )
        )

        data["obs"]["eye_in_hand_rgb"].append(
            ObsUtils.process_obs(
        ...
        data["obs"]["gripper_states"].append(
            torch.from_numpy(np.array(obs[k]["robot0_gripper_qpos"])).float()
        ),

        data["obs"]["joint_states"].append(
            torch.from_numpy(np.array(obs[k]["robot0_joint_pos"])).float()
        ),
...
def evaluate_one_task_success(
    ...
    for i in range(eval_loop_num):
        ...
        obs = (
            ...
            if env_num > 1
            else env.set_init_state(init_states_[0])
        )
        ...
        if task_str != "":
            ...
            for k in range(env_num):
                if i * env_num + k < cfg.eval.n_eval:
                    sim_states[i * env_num + k].append(
                        sim_state if env_num == 1 else sim_state[k]
                    )
...
def evaluate_multitask_training_success(
    cfg, algo, benchmark, task_ids, result_summary=None
):
    """
    Evaluate the success rate for all task in task_ids.
    """
    ...
    for i in task_ids:
        ...
        success_rate, curr_summary = evaluate_one_task_success(
            cfg=cfg,
            algo=algo,
            task=task_i,
            task_emb=task_emb,
