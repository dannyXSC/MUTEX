...
def get_audio_specification(benchmark_name, task_list, cfg, mode="train"):
    ...
    task_id_range = (
        range(train_max_ts)
        if mode == "train"
        else range(train_max_ts, cfg.n_ts_per_task)
    )
    ...
    for spec_type in ["ag", "ai"]:
        saved_emb_path = os.path.join(
            cfg.folder,
            benchmark_name,
            "task_spec",
            f"{spec_type}_{tokenizer_str}_ts_mode_{mode}_emb.pt",
        )
        if (not cfg.recalculate_ts_embs) and (os.path.exists(saved_emb_path)):
            print(
                f"[INFO]: Loading {spec_type} embeddings for ts mode {mode} from {saved_emb_path}"
            )
            ...
            continue
        print(
            f"[WARNING]: Recalculating {spec_type} embeddings for ts mode {mode}. This may create a mismatch with train and eval."
        )
        for task_name in task_list:
            ...
            for store_ind, ind in enumerate(task_id_range):
                audio_path = os.path.join(
                    ...
                    benchmark_name,
                    "speech",
                    task_name + f"_{spec_type}{ind:03d}.mp3",
                )
                # Load audio file
                ...
                encoder_feats = encoder_feats.view(
                    encoder_feats.size(0),
                    encoder_feats.size(1) // num_chunks,
                    num_chunks,
                    encoder_feats.size(-1),
                )
            ...
            spec_dict = {
                ...
                f"{spec_type}_task_spec_mask": torch.ones(
                    encoder_feats_final.shape[:-1]
                ),
            }
            if spec_type == "ai":
...
def get_visual_specifications_all(
    benchmark_name, task_list, task_demo_path_list, cfg, mode="train"
):
    train_max_ts = int(0.8 * cfg.n_ts_per_task)
    task_id_range = (
        range(train_max_ts)
    ...
    saved_emb_path = os.path.join(
        cfg.folder,
        task_demo_path_list[0][:-5].split("/")[0],
        "task_spec",
        f"visual_{tokenizer_str}_emb.pt",
    )
    return_dict = {}

    if cfg.recalculate_ts_embs or (not os.path.exists(saved_emb_path)):
        print(
            "[WARNING]: Recalculating visual embeddings. This may create mismatch between train and eval."
        )
        ...
        if cfg.visual_embedding_format == "clip":
            ...
            visual_emb_model.create_precomputable_models(
        ...
        for task_name, task_demo_path in zip(task_list, task_demo_path_list):
            vid_dir = os.path.join(
                cfg.folder,
                task_demo_path[:-5].split("/")[0],
                "task_spec",
                task_demo_path[:-5].split("/")[1],
            )
            img_task_spec_final, img_task_spec_mask_final = [], []
            vid_task_spec_final, vid_task_spec_mask_final = [], []
            for task_spec_id in range(
                cfg.n_ts_per_task
            ):  # Iterate over the different task specification examples, save everything
                ...
                if cfg.visual_embedding_format == "clip":
                    visual_task_spec = visual_preprocessor(
                        image_list, return_tensors="pt", padding=True
                    )["pixel_values"]
                elif cfg.visual_embedding_format == "r3m":
                    image_list = [
                        torch.tensor(np.array(im)).permute(2, 0, 1) for im in image_list
                    ]
                ...
                for i in range(0, visual_task_spec.shape[0], batch_size):
                    ...
                    with torch.no_grad():
                        output_batch = visual_emb_model.pre_compute_feats(input_batch)[
                            0
                        ]  ## [bs, 50, 512] 0th index has hidden embeds
                        vid_feats = visual_emb_model.post_compute_feats(output_batch)[
                            "image_embeds"
                        ]  ## [bs, 512]
                    visual_task_spec_list.append(output_batch)  ## for images
                    vid_task_spec_feat_list.append(
                        vid_feats.detach()
                    )  ## stores the features, for videos

                visual_task_spec = torch.cat(
                    visual_task_spec_list, dim=0
                )  ## [T, 50, 512]
                img_task_spec_feat = (
                    visual_task_spec[-1:, :, :].detach().cpu()
                )  ## [1, 50, 512]
                vid_task_spec_feat = (
                    torch.cat(vid_task_spec_feat_list, dim=0).detach().cpu()
                )  ## [T, 512]
                img_task_spec_final.append(img_task_spec_feat)
                vid_task_spec_final.append(vid_task_spec_feat)
                img_task_spec_mask_final.append(
                    torch.ones(img_task_spec_feat.shape[:-1])
                )
                vid_task_spec_mask_final.append(
                    torch.ones(vid_task_spec_feat.shape[:-1])
                )
            return_dict[task_name] = (
                {  # Save using task_name to maintain consistency accrross systems
...
def get_task_embs(cfg, descriptions, spec_type, mode="train"):
    ...
    if mode == "train" or mode == "eval":
        ...
        task_id_range = (
            range(train_max_ts)
            if mode == "train"
            else range(train_max_ts, cfg.n_ts_per_task, 1)
        )
    else:
        raise NotImplementedError
    ...
    if cfg.recalculate_ts_embs or (not os.path.exists(saved_emb_path)):
        ...
        if cfg.lang_embedding_format == "clip":
            ...
            model = (
                T5ForConditionalGeneration.from_pretrained(cfg.lang_tokenizer)
                .get_encoder()
                .eval()
            )
        else:
        ...
        if spec_type == "inst":
            task_embs = torch.empty(
                (len(descriptions), len(task_id_range), cfg.data.max_instructs, 768)
            )
            tokens = {
                "input_ids": torch.empty(
                    (
                        len(descriptions),
                        len(task_id_range),
                        cfg.data.max_instructs,
                        cfg.data.max_word_len,
                    )
                ),
            ...
            for task_ind, descriptions_task in enumerate(descriptions):
                ...
                for store_ind, ind in enumerate(task_id_range):
                    ...
                    if token["attention_mask"].size(0) > cfg.data.max_instructs:
                        print(
                            "[ERROR] Number of instructions are more than maximum allowed for task:",
                            description,
                        )
                        raise Exception
                    elif token["attention_mask"].size(0) < cfg.data.max_instructs:
                        pad_len = cfg.data.max_instructs - token["attention_mask"].size(
                            0
                        )
                        for k, v in token.items():
                            token[k] = torch.cat(
                                (v, torch.zeros((pad_len, v.size(-1))).to(cfg.device)),
                                dim=0,
                            ).long()
                    if cfg.lang_embedding_format == "clip":
                    ...
                    tokens["attention_mask"][task_ind, store_ind] = token[
                        "attention_mask"
                    ]
            ...
            tokens = {
                "input_ids": torch.empty(
                    (len(descriptions), len(task_id_range), cfg.data.max_word_len)
                ),
                "attention_mask": torch.empty(
                    (len(descriptions), len(task_id_range), cfg.data.max_word_len)
                ),
            ...
            raise NotImplementedError
        tokens["mask_token_id"] = torch.zeros(
            (task_embs.shape[0],)
        ).long()  ## converts it to int64, there's not such thing as masking in clip
        # repeat stopword tokens for each task
        tokens["stopword_tokens"] = (
            torch.tensor(stopword_tokens)
            .long()
            .unsqueeze(0)
            .repeat(task_embs.shape[0], 1)
        )
