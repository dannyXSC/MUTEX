...
from mutex.embed_utils import (
    get_visual_specifications_all,
    get_task_embs,
    get_audio_specification,
)
...
def main(hydra_cfg):
    ...
    if "inst" in task_spec_modalities:
        ...
        inst_embs, inst_tokens = get_task_embs(
            cfg, dataset_inst, spec_type="inst", mode=mode
        )
        print("Done")
    ...
    datasets = [
        MLMTaskDataset(
            ...
            task_tokens=(
                {k: v[i] for k, v in task_tokens.items()}
                if task_tokens is not None
                else None
            ),
            gl_tokens=(
                {k: v[i] for k, v in gl_tokens.items()}
                if gl_tokens is not None
                else None
            ),
            inst_tokens=(
                {k: v[i] for k, v in inst_tokens.items()}
            ...
            visual_spec=(
                task_visual_specifications[i]
                if len(task_visual_specifications) > 0
                else None
            ),
            cfg=cfg,
        )
        ...
        for attempt in range(num_attempts):
            try:
                ...
                wandb.init(
                    ...
                    resume=not (cfg.ckpt_dir is None),
                    config=cfg,
                    mode="offline" if attempt == num_attempts - 1 else cfg.wandb_mode,
                )
