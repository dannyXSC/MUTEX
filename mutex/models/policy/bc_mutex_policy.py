...
from mutex.models.mlm_head import (
    MFMHead,
    MRMHead,
    MIMHead,
    MGMHead,
    ...
    def __init__(
        ...
        for proprio_dim, use_modality, modality_name in [
            ...
            if use_modality:
                ...
                self.extra_encoders[modality_name] = nn.ModuleDict(
...
class BCMutexPolicy(BasePolicy):
    ...
    def __init__(self, cfg, shape_meta):
        ...
        policy_cfg.projection_layer.network_kwargs.output_size = (
            policy_cfg.projection_layer.network_kwargs.input_size
        )
        ...
        if cfg.lang_embedding_format == "clip":
            self.language_emb_model = CLIPTextModelWithProjection.from_pretrained(
                cfg.lang_tokenizer, cache_dir=to_absolute_path("./clip")
            ).eval()
        ...
        if cfg.visual_embedding_format == "clip":
            self.visual_emb_model = CLIPVisionSliced.from_pretrained(
                cfg.tokenizer, cache_dir=to_absolute_path("./clip")
            )
            self.visual_emb_model.create_precomputable_models(
                layer_ind=cfg.policy.slice_model_ind
            )
        ...
        if self.add_mfm:
            ...
            self.mfm_head = eval(cfg.policy.mfm_head.network)(
                **cfg.policy.mfm_head.network_kwargs
            )
        if self.add_mrm:
            cfg.policy.mrm_head.network_kwargs.input_size = cfg.policy.embed_size
            self.mrm_head = eval(cfg.policy.mrm_head.network)(
                **cfg.policy.mrm_head.network_kwargs
            )
        if self.add_mim:
            ...
            self.mim_head = eval(cfg.policy.mim_head.network)(
                ...
                layer_norm_eps=(
                    ...
                    if hasattr(text_config, "layer_norm_eps")
                    else text_config.layer_norm_epsilon
                ),
        ...
        if self.add_magm:
            ...
            self.magm_head = eval(cfg.policy.magm_head.network)(
                **cfg.policy.magm_head.network_kwargs
            )
        if self.add_maim:
            cfg.policy.maim_head.network_kwargs.input_size = cfg.policy.embed_size
            self.maim_head = eval(cfg.policy.maim_head.network)(
                **cfg.policy.maim_head.network_kwargs
            )
        ...
        if self.debug:
            self.tz = AutoTokenizer.from_pretrained(
                cfg.lang_tokenizer, cache_dir=to_absolute_path("./bert")
            )
    ...
    def get_task_embs(self, data, modalities=None):
        if modalities is None:
            modalities = (
                self.task_spec_modalities
            )  ## IF not passed explicitly, pick all modalities
        ...
        if self.training:
            ...
            while (
                self.cross_modal_matching
                and (num_modalities == 1)
                and (selected_modalities[0] == "vid")
            ):
                num_modalities = random.randint(1, len(modalities))
                selected_modalities = random.sample(modalities, num_modalities)
        ...
        inst_emb, gl_emb, img_emb, vid_emb, ag_emb, ai_emb = (
            None,
            None,
            None,
            None,
        ...
        if "inst" in modalities:
            if "inst_emb" in data:
                ...
                inst_emb = self.language_emb_model(
                    ...
                    attention_mask=(
                        attention_mask.reshape(bs * t, -1)
                        if num_modalities > 1
                        else None
                    ),
                )[self.lang_output_key]
                if (
                    "hidden_state" in self.lang_output_key
                ):  # if we are taking hidden states, we need to take the mean of all the hidden tokens
        ...
        if "gl" in modalities:
            if "gl_emb" in data:
                gl_emb = data[
                    "gl_emb"
                ]  ## batch size should be handled in the other part
            else:
                gl_emb = self.language_emb_model(
                    input_ids=data["gl_tokens"]["input_ids"],
                    attention_mask=(
                        data["gl_tokens"]["attention_mask"]
                        if num_modalities > 1
                        else None
                    ),
                )[self.lang_output_key].unsqueeze(
                    dim=1
                )  ## [B, 1, E]
                if (
                    "hidden_state" in self.lang_output_key
        ...
        if "img" in modalities:
            ...
            img_spec = img_spec.reshape(
                B * T, img_spec.shape[2], img_spec.shape[3]
            )  # .to(self.device)
            ...
            img_emb = self.visual_emb_model.post_compute_feats(
                img_spec, attention_mask=img_spec_mask if num_modalities > 1 else None
        ...
        if "vid" in modalities:
            ...
            vid_emb_mask = (
                data["vid_spec_mask"]
                if num_modalities > 1
                else torch.ones(data["vid_spec_mask"].shape).to(vid_emb.device)
            )  ## don't mask if only one modality
            vid_emb = self.projection_layers.add_temporal_token(
                vid_emb
            )  ## It is called language encoder for backward compatibility
        if "ag" in modalities:
            ag_emb = data["ag_task_spec"]
            ag_emb_mask = (
                data["ag_task_spec_mask"]
                if num_modalities > 1
        ...
        emb, emb_mask, gt_rep = self.projection_layers.aggregate_task_embs(
            ...
            emb_mask_dict={
                "inst_emb_mask": inst_emb_mask,
                "gl_emb_mask": gl_emb_mask,
                "img_emb_mask": img_emb_mask,
                "vid_emb_mask": vid_emb_mask,
                "ag_emb_mask": ag_emb_mask,
                "ai_emb_mask": ai_emb_mask,
            },
        ...
        rep_dict = {
            "task_emb": emb.clone(),
            "gt_rep": gt_rep.clone() if gt_rep is not None else None,
        }
    ...
    def temporal_encode(self, x, context_tokens, context_tokens_mask=None):
        ...
        sh = x.shape
        self.temporal_transformer.compute_mask(
            x.shape,
            context_shape=context_tokens.shape,
            context_mask=context_tokens_mask,
        )
    ...
    def forward(self, data, reduction="mean"):
        data = self.process_input(
            data, train_mode=True
        )  ## Even for validation keep train_mode=True
        data["task_emb"], data["task_emb_mask"], inp_modalities, rep_dict = (
            self.get_task_embs(data)
        )
        x = self.spatial_encode(data)
        x = self.temporal_encode(
            x,
            context_tokens=data["task_emb"],
            context_tokens_mask=data["task_emb_mask"],
        )

        add_mrm = (
            self.add_mrm and (len(inp_modalities) > 1) and ("img" in inp_modalities)
        )
        add_mfm = (
            self.add_mfm and (len(inp_modalities) > 1) and ("vid" in inp_modalities)
        )
        add_mim = (
            self.add_mim and (len(inp_modalities) > 1) and ("inst" in inp_modalities)
        )
        add_mgm = (
            self.add_mgm and (len(inp_modalities) > 1) and ("gl" in inp_modalities)
        )
        add_maim = (
            self.add_maim and (len(inp_modalities) > 1) and ("ai" in inp_modalities)
        )
        add_magm = (
            self.add_magm and (len(inp_modalities) > 1) and ("ag" in inp_modalities)
        )
        ...
        dist = self.policy_head(
            x[:, query_meta["action_start_ind"] : query_meta["action_end_ind"], :]
        )
        ...
        if self.add_rep_loss and (rep_dict["gt_rep"] is not None):
            ...
            loss_rep = self.rep_loss_coef * criterion(
                task_emb, gt_rep.repeat(1, task_emb.shape[1], 1)
            )
            loss += loss_rep
            ...
            if self.debug:
                ...
                print(
                    "[debug] Rep Loss Orig: {}".format(
                        loss_rep.data / self.rep_loss_coef
                    )
                )
        ...
        if add_mim:
            ...
            if self.debug:
                print(
                    "[debug] MIM loss: {}, BC Loss: {}".format(loss_mim.data, loss.data)
                )
                mim_logits = mim_logits[
                    :, 0, :
                ]  ## just recreate the first masked token in the batch size
        ...
        if add_mgm:
            ...
            if self.debug:
                print(
                    "[debug] MGM loss: {}, BC Loss: {}".format(loss_mgm.data, loss.data)
                )
                mgm_logits = mgm_logits[
                    :, 0, :
                ]  ## just recreate the first masked token in the batch size
    ...
    def get_action(self, data):
        ...
        with torch.no_grad():
            ...
            ### the decoder
            q_x, cross_attn_mask, self_attn_mask, query_meta = (
                self.decoder.get_query_vec(
                    num_input_tokens=x.shape[1],
                    batch_size=x.shape[0],
                    lang_q_ind=[],
